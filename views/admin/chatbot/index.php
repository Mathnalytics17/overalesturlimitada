...
<style>
.chatbot-page{padding:18px}.chatbot-header{display:flex;justify-content:space-between;gap:16px;align-items:center;margin-bottom:18px}.chatbot-title h1{font-size:26px;margin:0;color:#183b3f}.chatbot-title p{margin:6px 0 0;color:#667085}.chatbot-card{background:#fff;border:1px solid #e5e7eb;border-radius:16px;box-shadow:0 8px 24px rgba(15,23,42,.05);padding:16px;margin-bottom:16px}.chatbot-table-card{overflow:auto}.chatbot-filters{display:grid;grid-template-columns:2fr 1fr 1fr .7fr auto;gap:12px;align-items:end}.chatbot-filters label{font-size:13px;font-weight:700;color:#344054}.chatbot-filters input,.chatbot-filters select{width:100%;padding:10px 12px;border:1px solid #d0d5dd;border-radius:10px}.chatbot-btn{display:inline-flex;align-items:center;justify-content:center;gap:8px;border:0;border-radius:10px;padding:10px 14px;text-decoration:none;font-weight:700;cursor:pointer}.chatbot-btn-primary{background:#488B8F;color:white}.chatbot-btn-light{background:#f2f4f7;color:#344054}.chatbot-btn-success{background:#12b76a;color:white}.chatbot-table{width:100%;min-width:760px;border-collapse:collapse}.chatbot-table th{background:#f8fafc;color:#475467;font-size:12px;text-transform:uppercase;text-align:left;padding:12px}.chatbot-table td{border-top:1px solid #eef2f6;padding:12px;color:#344054;vertical-align:middle}.badge{display:inline-flex;border-radius:999px;padding:4px 10px;font-size:12px;font-weight:800}.badge-success{background:#dcfae6;color:#067647}.badge-danger{background:#fee4e2;color:#b42318}.badge-warning{background:#fef0c7;color:#b54708}.empty{text-align:center;padding:30px;color:#667085}.alert{padding:12px 14px;border-radius:12px;background:#fff3cd;color:#664d03;margin-bottom:16px}@media(max-width:900px){.chatbot-header,.chatbot-filters{display:block}.chatbot-filters>*{margin-bottom:10px}.chatbot-table{font-size:13px}.chatbot-page{padding:10px}}@media(max-width:760px){.chatbot-table-card{overflow:visible}.chatbot-table{min-width:0}.chatbot-btn{width:100%}}
</style>
...
<div class="chatbot-page">
    ...
    <div class="chatbot-card">
        <form class="chatbot-filters" method="get" action="/admin/chatbot">
            ...
            <div><label>Filas</label><select name="per_page"><?php foreach ([10, 25, 50, 100] as $size): ?><option value="<?= $size ?>" <?= (int)($pagination['per_page'] ?? 50) === $size ? 'selected' : '' ?>><?= $size ?></option><?php endforeach; ?></select></div>
            <button class="chatbot-btn chatbot-btn-primary" type="submit">Filtrar</button>
        </form>
    </div>

    <div class="chatbot-card chatbot-table-card">
        <table class="chatbot-table">
            ...
            <?php foreach ($contacts as $c): [$label,$class]=consent_badge($c); $id=$c['id'] ?? ''; $session=$c['bot_session'] ?? ''; ?>
                <tr>
                    <td data-label="Contacto"><strong><?= h($c['name'] ?? $c['notify_name'] ?? 'Sin nombre') ?></strong><br><small><?= h($c['phone_number'] ?? $c['phone'] ?? $c['from'] ?? '') ?></small></td>
                    <td data-label="Sesión"><?= h($session) ?></td>
                    <td data-label="Política"><span class="badge badge-<?= h($class) ?>"><?= h($label) ?></span></td>
                    <td data-label="Estado"><?= h($c['state_name'] ?? $c['current_state'] ?? $c['status'] ?? '-') ?></td>
                    <td data-label="Último mensaje"><?= h($c['last_message_at'] ?? $c['updated_at'] ?? $c['created_at'] ?? '-') ?></td>
                    <td data-label="Acciones"><a class="chatbot-btn chatbot-btn-light" href="/admin/chatbot/show?id=<?= urlencode((string)$id) ?>&bot_session=<?= urlencode((string)$session) ?>">Ver chat</a></td>
                </tr>
            <?php endforeach; ?>
            </tbody>
        </table>
        <?php
          $paginationBaseUrl = '/admin/chatbot';
