...
$travelSummaryText = implode(' Â· ', $travelSummary);
...
<style>
  ...
  .lead-detail-form textarea,
  .lead-detail-form input,
  .lead-detail-form select {
    margin:10px 0;
  }
  ...
  <div class="lead-detail-layout">
      ...
      <div class="card">
        <div class="card-head">
          <div>
            ...
            <div class="card-sub"><?= e((string) ($lead->source_type ?? '')) ?> Â· <?= e((string) ($lead->status ?? '')) ?></div>
          </div>
        </div>
        ...
        <div class="lead-detail-meta">
          ...
          <div><strong>TelÃ©fono:</strong><br><?= e((string) ($lead->phone ?? '')) ?></div>
          <div>
        ...
        <?php if ($isTickets): ?>
          <div class="lead-detail-section">
            ...
            <div class="lead-detail-grid" style="margin-top:10px;">
        ...
        <?php if ($isPackage): ?>
          <div class="lead-detail-section">
            ...
            <div class="lead-detail-box" style="margin-top:10px;">
        ...
        <?php if (!empty($showMetadata)): ?>
          <div class="lead-detail-section">
            ...
            <div class="lead-detail-body" style="background:#fff;">
              <?php foreach ($showMetadata as $key => $value): ?>
                <div style="margin-bottom:6px;">
                  <strong><?= e((string) $key) ?>:</strong>
                  <?= e(is_bool($value) ? ($value ? 'SÃ­' : 'No') : (string) $value) ?>
                </div>
              <?php endforeach; ?>
            </div>
          </div>
          ...
          <div class="lead-detail-list">
            <?php foreach ($interactions as $item): ?>
              <div class="lead-detail-item">
                <div class="lead-detail-small">
                  <?= e((string) $item->created_at) ?> Â· <?= e((string) $item->event_type) ?> Â· <?= e((string) $item->channel) ?>
                </div>
            ...
            <?php if ($interactions === []): ?>
              <div>No hay movimientos registrados todavÃ­a.</div>
            <?php endif; ?>
          </div>
        </div>
      </div>
    ...
    <div class="lead-detail-sidebar">
      <?php if (!$isAssigned): ?>
        <div class="card">
          <h3>Tomar caso</h3>
          <p class="lead-detail-muted" style="margin:0 0 12px 0;">
            Este caso aÃºn no tiene responsable. Puedes tomarlo para gestionarlo.
          </p>

          <form method="post" action="/admin/leads/take" class="lead-detail-actions">
            <?= \app\Core\Csrf::input(); ?>
      ...
      <?php elseif ($isMine): ?>
        <div class="card">
          ...
          <p style="margin:0 0 12px 0;color:#16a34a;font-weight:700;">
            Este caso estÃ¡ asignado a ti.
          </p>

          <?php if (!in_array((string) ($lead->status ?? ''), ['closed', 'lost'], true)): ?>
            <form method="post" action="/admin/leads/release" class="lead-detail-actions">
              <?= \app\Core\Csrf::input(); ?>
              <input type="hidden" name="lead_id" value="<?= (int) ($lead->id ?? 0) ?>">
              <input type="hidden" name="return_to" value="<?= e($returnTo) ?>">
      ...
      <?php else: ?>
        <div class="card">
          <h3>Responsabilidad del caso</h3>
          <p class="lead-detail-muted" style="margin:0;">
      ...
      <div class="card">
        ...
        <form method="post" action="/admin/leads/status" class="lead-detail-form">
          <?= \app\Core\Csrf::input(); ?>
          <input type="hidden" name="lead_id" value="<?= (int) ($lead->id ?? 0) ?>">
          <input type="hidden" name="return_to" value="<?= e($returnTo) ?>">
      ...
      <div class="card">
        <h3>Agregar nota</h3>
        <form method="post" action="/admin/leads/note" class="lead-detail-form">
          <?= \app\Core\Csrf::input(); ?>
          <input type="hidden" name="lead_id" value="<?= (int) ($lead->id ?? 0) ?>">
          <input type="hidden" name="return_to" value="<?= e($returnTo) ?>">
      ...
      <div class="card">
        ...
        <form method="post" action="/admin/leads/task" class="lead-detail-form" style="margin-bottom:14px;">
          <?= \app\Core\Csrf::input(); ?>
          <input type="hidden" name="lead_id" value="<?= (int) ($lead->id ?? 0) ?>">
          <input type="hidden" name="return_to" value="<?= e($returnTo) ?>">
        ...
        <div class="lead-detail-list" style="margin-top:0;">
      ...
      <div class="card">
        ...
        <?php else: ?>
          <span class="lead-detail-muted" style="font-weight:700;">No comercial</span>
        <?php endif; ?>
      </div>
    </div>
  </div>
