...
if (!function_exists('pagination_page')) {
    function pagination_page(mixed $value): int
    {
...
if (!function_exists('pagination_per_page')) {
    function pagination_per_page(mixed $value, array $allowed = [10, 20, 50, 100], int $default = 20): int
    {
...
if (!function_exists('pagination_url')) {
    function pagination_url(array $query = [], array $overrides = []): string
    {
...
if (!function_exists('render_pagination')) {
    function render_pagination(array $pagination, array $query = []): string
    {
        ...
        $prevUrl = pagination_url($query, ['page' => (int) ($pagination['prev_page'] ?? max(1, $page - 1)), 'per_page' => $perPage]);
        $nextUrl = pagination_url($query, ['page' => (int) ($pagination['next_page'] ?? min($pages, $page + 1)), 'per_page' => $perPage]);

        $html = '<div class="pagination">';
        $html .= '<span>Mostrando ' . $from . '-' . $to . ' de ' . $total . ' | Pagina ' . $page . ' de ' . $pages . '</span>';
        $html .= '<div class="pager">';

        if (!empty($pagination['has_prev'])) {
            $html .= '<a class="btn small" href="' . e($prevUrl) . '" style="text-decoration:none;">Anterior</a>';
        } else {
            $html .= '<span class="btn small" style="opacity:.55;cursor:default;">Anterior</span>';
        }

        if (!empty($pagination['has_next'])) {
            $html .= '<a class="btn small" href="' . e($nextUrl) . '" style="text-decoration:none;">Siguiente</a>';
        } else {
            $html .= '<span class="btn small" style="opacity:.55;cursor:default;">Siguiente</span>';
        }
